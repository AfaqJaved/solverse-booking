...
export const businessClient = {
	register: async (data: BusinessApi.Register.Request): Promise<BusinessApi.Register.Response> => {
		const response = await apiClient.post<BusinessApi.Register.Response>(
			APICONSTANTS.BUSINESSES.CREATE,
			data
		)
	...
	getByOwner: async (ownerId: string): Promise<BusinessApi.GetBusiness.Response[]> => {
		const response = await apiClient.get<BusinessApi.GetBusiness.Response[]>(
			APICONSTANTS.BUSINESSES.GET_BY_OWNER(ownerId)
		)
		return response.data
	},
	...
	getById: async (businessId: string): Promise<BusinessApi.GetBusiness.Response> => {
		const response = await apiClient.get<BusinessApi.GetBusiness.Response>(
			APICONSTANTS.BUSINESSES.GET_BY_ID(businessId)
		)
		return response.data
	},
	...
	updateProfile: async (
		businessId: string,
		data: BusinessApi.UpdateProfile.Request
	): Promise<void> => {
