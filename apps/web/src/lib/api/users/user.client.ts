...
export const userClient = {
  register: async (data: UserApi.Register.Request): Promise<UserApi.Register.Response> => {
    const response = await apiClient.post<UserApi.Register.Response>(APICONSTANTS.USERS.REGISTER, data)
    return response.data
  },

  getProfile: async (userId: string): Promise<UserApi.GetUser.Response> => {
    const response = await apiClient.get<UserApi.GetUser.Response>(APICONSTANTS.USERS.GET_BY_ID(userId))
    return response.data
  },
